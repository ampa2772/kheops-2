...
| A17-19 (volet 2) | **Microsoft Graph étendu : agenda + contacts** (OneDrive fait via A3) | ✅ serveur livré le 2026-07-02 : lecture agenda + contacts Outlook (`microsoftGraphExtended.js` + routes `/api/microsoft/*`), réutilise l'auth MS existante, scopes `Calendars.Read`+`Contacts.Read` ajoutés (⚠️ imposent UNE reconnexion aux comptes MS existants), +32 tests (serveur 443, client 1346) ; service client prêt (`microsoftGraphClient.js`). RESTE (🅱️) : affichage à l'écran (validation navigateur) |
