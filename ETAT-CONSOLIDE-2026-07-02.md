...
| A8/A9 | Passe `npm audit` serveur + client + supprimer `express-session` | ✅ livré le 2026-07-03 : **serveur 40 → 0** (express-session retiré ; audit fix ; sauts majeurs testés un à un : nodemailer 6→9, msal-node 1→5, @googleapis/gmail 12→17, nodemon 2→3 ; overrides ciblés uuid ^11.1.1 dans la chaîne GCS — surtout NE PAS suivre la « correction » npm qui rétrograde @google-cloud/storage 7→5) ; **client 67 → 19** (audit fix + overrides nth-check/postcss/serialize-javascript). Résiduel client accepté et documenté : outillage de FABRICATION uniquement (react-scripts 5 : svgo/workbox/webpack-dev-server/sockjs + jest/jsdom), pas de code livré au navigateur — se résoudra en sortant de CRA (chantier séparé). Vérifs : serveur 458/458 + démarrage réel (health 200) ; client 1359/1359 + build de production |
...
| A17-19 (volet 2) | **Microsoft Graph étendu : agenda + contacts** (OneDrive fait via A3) | ✅ livré le 2026-07-02 : serveur (`microsoftGraphExtended.js` + routes `/api/microsoft/*`, lecture seule, scopes `Calendars.Read`+`Contacts.Read` ajoutés → ⚠️ UNE reconnexion requise pour les comptes MS existants) + service client + **carte « Agenda Outlook » dans la barre latérale de l'agenda** (cachée sans compte MS — vérifié en navigateur : appel → 401 → retrait silencieux, page intacte). +38 tests (serveur 443, client 1352). L'annuaire a aussi son onglet « Outlook » (consultation seule, pagination, filtre par la recherche ; onglet absent sans compte MS). RESTE : éventuel IMPORT des contacts Outlook vers Kheops → décision produit (attention : pas de suppression de contacts → un import raté laisse des doublons définitifs) |
| A20 | Aide juridictionnelle : validation de schéma backend | ✅ livré le 2026-07-02 : `validation/aideJuridictionnelleSchema.js` (sections connues seules, contenus bornés, clés dangereuses exclues — piège : `constructor` passe une simple classe de caractères) branché via `validateBody` sur le PUT ; +8 tests |
| A21 | CARPA : audit trail des documents | ✅ livré le 2026-07-02 : l'ajout ET le retrait de pièce tracent désormais le `documentId` attaché (`champsModifies.piece`) — une substitution de fichier devient détectable ; +3 tests |
| A22 | `propagateContactToDivorces` : chemin *remove* | ✅ livré le 2026-07-02 : retrait des entrées PROPAGÉES (`pchId`) plus rattachées à AUCUN des deux époux (enfants + adultes à charge) ; entrées manuelles jamais touchées ; jamais de retrait si encore rattachée à l'autre conjoint ; +4 tests. Suite serveur : 59 suites / 458 verts |
...
- ✅ **Actions documents en mode web pur (2026-07-03)** : le bouton « Créer document » aboutit déjà en web (génération serveur `/api/word/:id/generate`, modèles déposés) ; AJOUTÉ : **téléchargement navigateur** du .docx (`wordDocumentClient.js` → `GET /api/word/:id/download`), branché sur le bouton Télécharger ET en repli quand le compagnon est absent ; **marche arrière web** si la génération échoue (plus de document fantôme) ; route morte `fusion/updateDocumentUrl` qui faisait pendre les requêtes → 410. +16 tests (serveur 459, client 1369). Vérifié : app se charge sans erreur imputable au code. **Document vierge recâblé (2026-07-03)** : bouton « Document Word vierge » dans la barre d'outils du dossier + route `POST /api/word/:docId/create-blank` (squelette OOXML fabriqué en mémoire, aucun modèle à provisionner) + marche arrière web ; icône Télécharger élargie aux .docx (était limitée aux .txt). **Vérifié de bout en bout en navigateur réel** : clic bouton → fiche 201 → create-blank 200 → toast → téléchargement 200. Duplication/suppression du FICHIER physique déjà couvertes côté serveur.
- ✅ **Finitions messagerie (2026-07-03)** : (1) **aperçu + téléchargement des PJ IMAP** dans la cloche (bascule `source==='imap'` → `/api/mail/messages/:id/attachments/:index` ; réutilise le pipeline d'aperçu image/PDF/docx/texte existant ; aucun endpoint serveur ajouté — le Content-Disposition est sans effet sur un fetch mémoire) ; (2) **badge du type de boîte** (Gmail rouge / Outlook bleu / IMAP gris) dans la page Courriers ET la cloche — **vérifié en navigateur** (badge « Gmail » aux deux endroits) ; (3) **TTL du cache e-mails** (6 h, `cachedKheopsEmailsAt` + `isEmailCacheStale`, purge au logout). +13 tests (serveur 462, client 1380).
